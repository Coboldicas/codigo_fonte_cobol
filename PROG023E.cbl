      ...
      * DATA.......: 20/10/2025
      * DESCRICAO..: CONTROLE DE ESTOQUE - ESTOQUE MINIMO 
      * NOME.......:   PROG023E
      * 
      * OBS........: ESSE PROGRAMA E UM CLONE DAS VERSOES ANTERIORES:
      *              PROG023A: VERSAO INICIAL
      *              PROG023B: EXCLUIR PRODUTO
      *              PROG023C: GRAVAR ARQUIVO DE LOG
      *              PROG023D: EXCLUSAO LOGICA
      ******************************************************************
      ...
      *================================================================*
       ...
       FILE-CONTROL.
           ...
           SELECT ARQUIVO-SAI ASSIGN TO 'ARQLOG1.dat'
               ORGANIZATION IS LINE SEQUENTIAL.
       ...
       FD  ARQUIVO-ENT.
       01  FD-ARQ-ENT.
         05  FDE-NUM-SEQUENCIAL           PIC 9(05) VALUE ZEROS.
         ...
         05  FDE-PROD-STATUS              PIC X(01) VALUE SPACES.

       FD  ARQUIVO-SAI.
       01  FD-ARQ-SAI.
         05  FDS-DATA-PROC                PIC X(10) VALUE SPACES.
         05  FDS-BARRA1                   PIC X(01) VALUE SPACES.
         05  FDS-HORA-PROC                PIC X(05) VALUE SPACES.
         05  FDS-BARRA2                   PIC X(01) VALUE SPACES.
         05  FDS-TRANSACAO                PIC X(10) VALUE SPACES.
         05  FDS-BARRA3                   PIC X(01) VALUE SPACES.
         05  FDS-COD-PRODUTO              PIC 9(05) VALUE ZEROS.
         05  FDS-BARRA4                   PIC X(01) VALUE SPACES.
         05  FDS-NOME-PRODUTO             PIC X(20) VALUE SPACES.
         05  FDS-BARRA5                   PIC X(01) VALUE SPACES.
         05  FDS-QTDE-PROD-ANT            PIC 9(05) VALUE ZEROS.
         05  FDS-BARRA6                   PIC X(01) VALUE SPACES.
         05  FDS-QTDE-PROD-ATU            PIC 9(05) VALUE ZEROS.
         05  FDS-BARRA7                   PIC X(01) VALUE SPACES.
         05  FDS-VALOR-PROD-ANT           PIC 9(15)V99 VALUE ZEROS.
         05  FDS-BARRA8                   PIC X(01) VALUE SPACES.
         05  FDS-VALOR-PROD-ATU           PIC 9(15)V99 VALUE ZEROS.
       ...
       01  WRK-QTDE-MIN                   PIC 9(05) VALUE ZEROS.
       01  WRK-VALOR-PRODUTO              PIC 9(15)V99 VALUE ZEROS.
       01  WRK-PROD-STATUS                PIC X(01) VALUE SPACES.
       ...
       01  WRK-VAL-QTDE-MIN               PIC X(01) VALUE 'N'. 
      ...
      *    LÊ O ARQUIVO E CARREGA NA TABELA INTERNA
           PERFORM VARYING WRK-IND1 FROM 1 BY 1 UNTIL 
               ...
               IF WRK-FIM-ARQ          EQUAL 'N'
                  MOVE WRK-IND1        TO TAB-NUM-SEQ(WRK-IND1)
                  ...
                  MOVE FDE-QTDE-MINIMA
                                       TO TAB-QTDE-MIN(WRK-IND1)
                  MOVE FDE-VALOR-PRODUTO
                                       TO TAB-VALOR(WRK-IND1)
                  MOVE FDE-PROD-STATUS TO TAB-PROD-STATUS(WRK-IND1)
      ...
      *----------------------------------------------------------------*
           ...
           PERFORM 0035-VALIDAR-QTDE-MIN
                                     UNTIL WRK-VAL-QTDE-MIN EQUAL 'S'
           ...
           MOVE WRK-QTDE-REG           TO TAB-NUM-SEQ(WRK-QTDE-REG)
           ...
           MOVE WRK-QTDE-MIN           TO TAB-QTDE-MIN(WRK-QTDE-REG)
           MOVE WRK-VALOR-PRODUTO      TO TAB-VALOR(WRK-QTDE-REG)           
           MOVE 'S'                    TO TAB-PROD-STATUS(WRK-QTDE-REG)           
      ...
      *    VALIDACAO CAMPO NOME PRODUTO
      *----------------------------------------------------------------*
       0035-VALIDAR-QTDE-MIN           SECTION.
      *----------------------------------------------------------------*

           DISPLAY '* QTDE. MINIMA:  'WITH NO ADVANCING
           ACCEPT WRK-QTDE-MIN

           IF WRK-QTDE-MIN            EQUAL ZEROS
              DISPLAY 'QUANTIDADE MINIMA NAO PODE SER ZERADO'
           ELSE
              MOVE 'S'                TO WRK-VAL-QTDE-MIN
           END-IF 
           .
      *----------------------------------------------------------------*
      ...
      *----------------------------------------------------------------*
           ...
           IF WRK-ENCONTROU            EQUAL 'S'
              DISPLAY '* 'TAB-CODIGO(WRK-IND1) ' | '
                          TAB-NOME(WRK-IND1)   ' | ' 
                          TAB-QTDE(WRK-IND1)   ' | '
                          TAB-QTDE-MIN(WRK-IND1) ' | '
                          TAB-VALOR(WRK-IND1)  ' | '
                          TAB-PROD-STATUS(WRK-IND1)  '     *'

              IF TAB-PROD-STATUS(WRK-IND1) 
                                       EQUAL 'N'      
                 DISPLAY 'DESEJA ATIVAR O PRODUTO? (S/N)' 
                          WITH NO ADVANCING
                 ACCEPT WRK-PROD-STATUS 
                 IF WRK-PROD-STATUS    EQUAL 'S'
                    PERFORM 0042-ENTRADA-PROD
                 ELSE
                    DISPLAY 'NAO E PERMITIDO ENTRADA DE PRODUTO INATIVO'
                 END-IF
              ELSE
                 PERFORM 0042-ENTRADA-PROD
              END-IF
      ...
      *----------------------------------------------------------------*
      *    EFETIVAR ENTRADA DE PRODUTOS
      *----------------------------------------------------------------*
       0042-ENTRADA-PROD               SECTION.
      *----------------------------------------------------------------*

                 MOVE TAB-QTDE(WRK-IND1) 
                                       TO FDS-QTDE-PROD-ANT
                 MOVE TAB-VALOR(WRK-IND1)
                                       TO FDS-VALOR-PROD-ANT
      
                 PERFORM 0033-VALIDAR-QTDE
                                     UNTIL WRK-VAL-QTDE EQUAL 'S'
                 ADD WRK-QTDE-PRODUTO  TO TAB-QTDE(WRK-IND1)
                 MOVE TAB-QTDE(WRK-IND1)
                                       TO FDS-QTDE-PROD-ATU
      
                 PERFORM 0034-VALIDAR-VALOR
                                     UNTIL WRK-VAL-VALOR EQUAL 'S'
                 MOVE WRK-VALOR-PRODUTO
                                       TO TAB-VALOR(WRK-IND1)         
                                          FDS-VALOR-PROD-ATU
                 MOVE 'S'              TO TAB-PROD-STATUS(WRK-IND1)
           ...
           IF WRK-ENCONTROU            EQUAL 'S'
              ...
              IF TAB-PROD-STATUS(WRK-IND1) 
                 ...
                 MOVE TAB-QTDE(WRK-IND1)
                                       TO FDS-QTDE-PROD-ANT
                 MOVE TAB-VALOR(WRK-IND1)
                                       TO FDS-VALOR-PROD-ATU
      
                 PERFORM 0033-VALIDAR-QTDE
                                    UNTIL WRK-VAL-QTDE EQUAL 'S'
      
                 SUBTRACT WRK-QTDE-PRODUTO
                                     FROM TAB-QTDE(WRK-IND1)

                 IF TAB-QTDE(WRK-IND1) EQUAL TAB-QTDE-MIN(WRK-IND1)
                    DISPLAY 'ALERTA: ESTOQUE NO NIVEL MINIMO'
                    DISPLAY '* 'TAB-CODIGO(WRK-IND1) ' | '
                          TAB-NOME(WRK-IND1)   ' | ' 
                          TAB-QTDE(WRK-IND1)   ' | '
                          TAB-QTDE-MIN(WRK-IND1) ' | '
                          TAB-PROD-STATUS(WRK-IND1)  '     *'
                 ELSE 
                    IF TAB-QTDE(WRK-IND1) LESS THAN 
                                        TAB-QTDE-MIN(WRK-IND1)
                    DISPLAY 'ALERTA: ESTOQUE ABAIXO DO NIVEL MINIMO'
                    DISPLAY '* 'TAB-CODIGO(WRK-IND1) ' | '
                          TAB-NOME(WRK-IND1)   ' | ' 
                          TAB-QTDE(WRK-IND1)   ' | '
                          TAB-QTDE-MIN(WRK-IND1) ' | '
      ...
      *----------------------------------------------------------------*
           ...
           PERFORM VARYING WRK-IND1 FROM 1 BY 1 UNTIL 
                                    WRK-IND1 > WRK-QTDE-REG
                  MOVE WRK-IND1        TO FDE-NUM-SEQUENCIAL
                  ...
                  MOVE TAB-QTDE(WRK-IND1)
                                       TO FDE-QTDE-PRODUTO
                  MOVE TAB-QTDE-MIN(WRK-IND1)
                                       TO FDE-QTDE-MINIMA
                  MOVE TAB-VALOR(WRK-IND1)
                                       TO FDE-VALOR-PRODUTO
                  MOVE TAB-PROD-STATUS(WRK-IND1)
                                       TO FDE-PROD-STATUS
                  WRITE FD-ARQ-ENT
           END-PERFORM
      ...
      *----------------------------------------------------------------*
           ...
           IF WRK-ENCONTROU          EQUAL 'S'
              ...
              IF TAB-PROD-STATUS(WRK-IND1)
                 ...
                 DISPLAY 'DESEJA EXCLUIR O CADASTRO DO PRODUTO? (S/N)'
                 ACCEPT WRK-EXCLUIR      
                 IF WRK-EXCLUIR        EQUAL 'S'
              ...
              ELSE
                 ...
                 IF WRK-EXCLUIR         EQUAL 'S'
                    ...
                    MOVE ZEROS          TO FDS-QTDE-PROD-ATU 
                    PERFORM 0009-GRAVAR-LOG
                    MOVE 'N'            TO TAB-PROD-STATUS(WRK-IND1) 
                 ELSE
                    CONTINUE
                 END-IF
              END-IF
           ...
           PERFORM VARYING WRK-IND1 FROM 1 BY 1 UNTIL 
              ...
              IF WRK-COD-PRODUTO    EQUAL TAB-CODIGO(WRK-IND1)
                 ...
                 MOVE TAB-VALOR(WRK-IND1 + 1)
                                       TO TAB-VALOR(WRK-IND1)
                 MOVE TAB-PROD-STATUS(WRK-IND1 + 1)
                                       TO TAB-PROD-STATUS(WRK-IND1)
              ELSE 
                 IF WRK-ENCONTROU EQUAL 'S'
      ...
      *----------------------------------------------------------------*
           ...
           MOVE WRK-MASC-DATA          TO FDS-DATA-PROC
           MOVE WRK-MASC-TIME          TO FDS-HORA-PROC

           IF WRK-OPCAO                EQUAL 1 
              MOVE 'CADASTRAR'         TO FDS-TRANSACAO
              MOVE WRK-COD-PRODUTO     TO FDS-COD-PRODUTO
              MOVE WRK-NOME-PRODUTO    TO FDS-NOME-PRODUTO
              MOVE ZEROS               TO FDS-QTDE-PROD-ANT
              MOVE WRK-QTDE-PRODUTO    TO FDS-QTDE-PROD-ATU
              MOVE ZEROS               TO FDS-VALOR-PROD-ANT  
              MOVE WRK-VALOR-PRODUTO   TO FDS-VALOR-PROD-ATU
           END-IF

           IF WRK-OPCAO                EQUAL 2 
              MOVE 'ENTRADA'           TO FDS-TRANSACAO
              MOVE WRK-COD-PRODUTO     TO FDS-COD-PRODUTO
              MOVE TAB-NOME(WRK-IND1)  TO FDS-NOME-PRODUTO
           END-IF

           IF WRK-OPCAO                EQUAL 3 
              MOVE 'SAIDA'             TO FDS-TRANSACAO
              MOVE WRK-COD-PRODUTO     TO FDS-COD-PRODUTO
              MOVE TAB-NOME(WRK-IND1)  TO FDS-NOME-PRODUTO
              MOVE ZEROS               TO FDS-VALOR-PROD-ANT  
           END-IF
           ...
           IF WRK-OPCAO                EQUAL 5 
              MOVE 'EXCLUIR'           TO FDS-TRANSACAO
              MOVE WRK-COD-PRODUTO     TO FDS-COD-PRODUTO
              MOVE TAB-NOME(WRK-IND1)  TO FDS-NOME-PRODUTO
              MOVE ZEROS               TO FDS-QTDE-PROD-ATU
              MOVE ZEROS               TO FDS-VALOR-PROD-ATU   
           END-IF
